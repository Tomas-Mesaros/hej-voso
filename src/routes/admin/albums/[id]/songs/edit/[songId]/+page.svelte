<script>
    ...
    onMount(async () => {
        await loadAlbumAndSong();
    });

    async function loadAlbumAndSong() {
        try {
            // Load album
            ...
            if (songResponse.ok) {
                ...
                if (song) {
                    ...
                    alert('Skladba sa nenašla');
                    goto(`/admin/albums/${albumId}/songs`);
                }
            }
        } catch (error) {
            console.error('Error loading album and song:', error);
            alert('Chyba pri načítaní skladby');
        } finally {
            isLoading = false;
        }
    }
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            if (response.ok) {
                alert('Skladba bola úspešne aktualizovaná');
                goto(`/admin/albums/${albumId}/songs`);
            } else {
                const error = await response.json();
                alert('Chyba pri aktualizácii skladby: ' + (error.message || 'Neznáma chyba'));
            }
        } catch (error) {
            console.error('Error updating song:', error);
            alert('Chyba pri aktualizácii skladby');
        } finally {
            isSubmitting = false;
        }
    }
...
<svelte:head>
    <title>Upraviť skladbu - {album?.title || 'Album'} - Admin</title>
</svelte:head>
                    ...
                    <div class="audio-upload">
                        {#if audioFile}
                            <div class="audio-preview">
                                ...
                                <button type="button" class="remove-audio" on:click={removeAudioFile}>
                        ...
                        {:else if originalFilename}
                            <div class="current-audio">
                                <i class="fas fa-file-audio"></i>
                                <span>Aktuálny súbor: {originalFilename}</span>
                                <audio controls>
                                    <source src="/uploads/audio/{originalFilename}" type="audio/mpeg">
                        ...
                        <div class="upload-controls">
                            ...
                            <label for="audio-upload" class="btn btn-outline">
                                ...
                                {originalFilename || audioFile ? 'Zmeniť súbor' : 'Nahrať audio'}
                            </label>
...
<style>
    /* Rovnaké štýly ako v add/+page.svelte s dodatočnými pre current-audio a remove-audio */
    .admin-container {
        max-width: 1000px;
        margin: 0 auto;
        padding: 20px;
    ...
    .audio-preview i, .current-audio i {
        font-size: 2rem;
        color: var(--button-color);
    }
